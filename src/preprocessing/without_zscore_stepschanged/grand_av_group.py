...
def save_individual_psd_plot(result: dict, output_dir: Path):
    """
    Saves the PSD plot for a single patient using data from the result dict.
    """
    patient_id = result["patient_id"]
    ...
    # Plot selected channels on a semilogy scale
    for ch in available:
        idx = ch_names.index(ch)
    ...
    ax.set_title(f"Patient {patient_id} - Grand Average PSD")
    ax.set_xlim([0, FMAX]) # FMAX is 80.0
    ax.legend(loc='upper right')
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    output_path = output_dir / f"{patient_id}_psd_grand_avg.png"
...
def main():
    ...
    for patient_id, patient_data in tqdm(patients.items(), desc="Computing PSDs"):
        ...
        if result is not None:
            ...
            try:
                save_individual_psd_plot(result, ind_dir)
                tqdm.write(f"  Saved plot for {patient_id}")
            except Exception as e:
                tqdm.write(f"  WARNING: Could not save plot for {patient_id}: {e}")
