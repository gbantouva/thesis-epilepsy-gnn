...
BACKWARD-COMPATIBLE VERSION - works with MNE < 0.23.0:
- Manual per-epoch detrending using scipy
- Zero-phase filtering (no time distortion)
- No z-score normalization (preserves amplitude relationships)

MODIFIED VERSION - with detrending visualization support:
- Added return_detrend_data parameter
- Returns epochs data before/after detrending for visualization

Key features:
- Spherical spline interpolation for missing channels (not zero-padding)
- Linear detrending PER EPOCH for MVAR model stationarity
- No ICA to preserve epileptiform activity
- 95th percentile artifact rejection
"""
...
def preprocess_single(
    ...
    return_detrend_data: bool = False,
    verbose: bool = True
) -> Optional[Dict]:
    """
    ...
    7. Notch filter (power line noise) with ZERO PHASE
    8. Bandpass filter (0.5-80Hz) with ZERO PHASE
    9. Resample (250 Hz)
    10. Common average reference (CAR)
    11. Epoch (4-second windows)
    12. PER-EPOCH linear detrending (manual using scipy)
    13. Artifact rejection (95th percentile + optional cap)
    14. NO z-score normalization (preserves amplitude relationships)
    ...
    Args:
        ...
        return_detrend_data: Whether to return epochs data before/after detrending
    ...
    Returns:
        ...
        - 'epochs_before_detrend': Epochs data before detrending (if return_detrend_data=True)
        - 'epochs_after_detrend': Epochs data after detrending (if return_detrend_data=True)
        - 'sampling_rate': Sampling rate in Hz (if return_detrend_data=True)
        - 'epoch_duration': Epoch duration in seconds (if return_detrend_data=True)
    ...
    # ========== 9. NOTCH FILTER (ZERO PHASE) ==========
    raw.notch_filter(freqs=[notch], phase='zero', verbose="ERROR")
    if verbose:
        print(f"✓ Notch filter at {notch} Hz (zero-phase)")
    
    # ========== 10. BANDPASS FILTER (ZERO PHASE) ==========
    # CRITICAL: phase='zero' prevents time distortion for connectivity
    raw.filter(l_freq=band[0], h_freq=band[1], 
               fir_design='firwin', 
               phase='zero',  # ⭐ CRITICAL FIX
               filter_length='auto', 
               verbose="ERROR")
    if verbose:
        print(f"✓ Bandpass filter {band[0]}-{band[1]} Hz (zero-phase)")
    ...
    # ========== 13. EPOCHING (WITHOUT detrend parameter for old MNE) ==========
    ...
    # ========== 14. MANUAL PER-EPOCH DETRENDING ==========
    # CRITICAL: Detrend each epoch individually using scipy
    # This works with any MNE version
    epochs_data = epochs.get_data()  # (n_epochs, n_channels, n_times)
    
    # SAVE DATA BEFORE DETRENDING (if requested for visualization)
    epochs_before_detrend = epochs_data.copy() if return_detrend_data else None
    
    for i in range(epochs_data.shape[0]):
        for ch in range(epochs_data.shape[1]):
            epochs_data[i, ch, :] = scipy.signal.detrend(
                epochs_data[i, ch, :], 
                type='linear'
            )
    
    # SAVE DATA AFTER DETRENDING (if requested for visualization)
    epochs_after_detrend = epochs_data.copy() if return_detrend_data else None
    
    # Create new epochs object with detrended data
    ...
    if verbose:
        print(f"✓ Applied per-epoch linear detrending")
    ...
    # ========== 16. NO Z-SCORE NORMALIZATION ==========
    # We skip z-scoring to preserve amplitude relationships for connectivity
    epochs_final = epochs_clean.copy()

    if verbose:
        print(f"✓ Skipped z-score normalization (preserves amplitude relationships)")
    ...
    # ADD DETRENDING DATA (if requested for visualization)
    if return_detrend_data:
        out["epochs_before_detrend"] = epochs_before_detrend
        out["epochs_after_detrend"] = epochs_after_detrend
        out["sampling_rate"] = resample_hz
        out["epoch_duration"] = epoch_len
